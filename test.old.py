"""Legacy single-process evaluation worker retained for reproducibility."""
...
# ---------------------------------------------------------------------------- #
#                     Server Communication (Port-Aware)                        #
# ---------------------------------------------------------------------------- #
session = requests.Session()

def job_online(device_name, dataset_size, log_dir, config, host: str, port: int):
...
def request_model_id(model_id, host: str, port: int):
    url = f"http://{host}:{port}/apply_model_id"
    response = session.post(url, json={"model_id": model_id})
    return response.json()["allowed"]


def report_result(model_id, result, host: str, port: int):
...
def finish_test(device_name, host: str, port: int):
    url = f"http://{host}:{port}/finish"
...
def test_model(
            ...
            while not outof_model or len(batch_model_id) > 0:
                try:
                    while len(batch_model_id) < batch_size:
                        ...
                        if not request_model_id(model_id, host, port):
                            pbar.update(1)
                            continue
                ...
                if len(batch_model_id) > 0:
                    ...
                    for model_id in finished_model:
                        report_result(model_id, results[model_id], host, port)
    ...
    finish_test(device_name=torch.cuda.get_device_properties(device).name, host=host, port=port)
